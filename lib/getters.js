...
module.exports = function (gm) {
  ...
  Object.keys(mapTerse).forEach(function (getter) {
    proto[getter] = function (opts, callback) {
      if (!callback) callback = opts, opts = {};

      var key = mapTerse[getter]
        , self = this;
      ...
      self.identifyTerse(opts, function (err, stdout, stderr, cmd) {
        if (err) {
          return callback.call(self, err, stdout, stderr, cmd);
        }
  ...
  proto.identifyTerse = function identifyTerse (opts, callback) {
    if (!callback) callback = opts, opts = {};
